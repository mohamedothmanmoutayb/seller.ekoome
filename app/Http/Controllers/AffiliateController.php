...
class AffiliateController extends Controller
{
    public function index(Request $request)
    {
        ...
        if(Auth::user()->id_role == 1){
            ...
            $customers = User::join('history_countries','history_countries.id_user','users.id')->with(['lead','fees' => function($query){
                $query = $query->with('countr');
            }])->where('id_role','11')->where('country_id', Auth::user()->country_id)->where('id_manager',Auth::user()->id);
        }
        ...
        $customers= $customers->where('id_role','11')->paginate('15');

        return view('backend.affiliate.index', compact('customers','agents','managers'));
    }
    ...
    public function situation(Request $request,$id)
    {
        $leads = Lead::join('lead_products','lead_products.id_lead','leads.id')
                        ...
                        ->whereIn('status_payment',['paid service','prepaid','return'])
                        ->where('id_user',$id)
                        ->where('type','affiliate')
                        ->select(\DB::raw('SUM(lead_products.lead_value) AS Value, leads.n_lead AS Lead ,leads.name AS name , leads.status_payment AS payment ,leads.id AS Id'),'leads.comission')
                        ->groupby('leads.created_at','leads.n_lead','leads.name','leads.status_payment','leads.id','leads.comission');
        if($request->items)
        {
            $items = $request->items;
        }else{
        ...
        return view('backend.affiliate.details', compact('leads','id','items'));
    }
    ...
    public function paied(Request $request)
    {
        ...
        foreach($ids as $v_id){
            ...
            if($id->status_livrison == "delivered"){
                ...
                $sum = $sum + $id->comission;  

                $id->status_payment = "paid";  
                $id->save();                          
            }
    ...
    public function fees($id)
    {
        ...
        return view('backend.affiliate.fees', compact('fees','countries','id'));
    }
    ...
    public function orders(Request $request,$id){
        $orders = Lead::where('id_user', $id)->where('status_livrison','delivered')->where('type','affiliate')->where('id_country', Auth::user()->country_id)->get();
        
        return view('backend.affiliate.orders', compact('orders','id'));
    }
    ...
    public function parameter($id)
    {
        ...
        return view('backend.affiliate.parameters', compact('parameter','id'));
    }
