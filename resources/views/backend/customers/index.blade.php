                        ...
                        <div class="modal-body">
                            <from class="form-horizontal form-material">
                                <div class="form-group">
                                    ...
                                    <div class="col-md-12 my-2">
                                        <input type="text" class="form-control" id="assigne_name" placeholder="Full Name">
                                    </div>
                    ...
                    <div class="row">
                        <div class="col-5 align-self-center ">
                            <h4 class="fw-bold py-3 mb-4 " style="display: -webkit-inline-box;"><span
                                    class="text-muted fw-light">Dashboard /</span> Customers list&nbsp;
                               
                            </h4>
                        </div> 
                        <div class="col-7 d-flex justify-content-end">
                            <div class="form-group mb-0 text-right">
                                 <a type="button" href="{{route('parrainage.index')}}" class="btn btn-primary btn-rounded text-white">Generate Referral links</a> 
                                 <a type="button" class="btn btn-primary btn-rounded waves-effect waves-light text-white" data-bs-toggle="modal" data-bs-target="#orderpaids">Paid Old Order</a>
                                <a type="button" class="btn btn-primary btn-rounded waves-effect waves-light text-white" data-bs-toggle="modal" data-bs-target="#orderpaid">Upload Order Paid</a>
                                <a type="button" class="btn btn-primary btn-rounded waves-effect waves-light text-white" data-bs-toggle="modal" data-bs-target="#adduser">Add New Customer</a>
                            </div>
                        </div>
                    </div>
                </div>

            <div id="orderpaids" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                                    ...
                                    <table class="table table-bordernone">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if(!empty($v_customer['manager']))
                                                    {{ $v_customer['manager']->name}}
                                                    @endif
                                                </td>
                                                ...
                                                <td>{{ $v_customer->Lastinvoice($v_customer->id)}}</td>
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end" aria-labelledby="earningReports">
                                                            @if(Auth::user()->id_role == 1 )
                                                                ...
                                                                <a class="dropdown-item" href="{{ route('customers.active', $v_customer->id)}}" > Active</a>
                                                                @else
                                                                <a class="dropdown-item" href="{{ route('customers.inactive', $v_customer->id)}}" > Inactive</a>
                                                                @endif
                                                                <a class="dropdown-item" href="{{ route('customers.situation', $v_customer->id)}}" > Order need to paid</a>
                                                                <a class="dropdown-item" href="{{ route('customers.orders', $v_customer->id)}}" > All orders</a>
                                                            @endif
                                                            <a class="dropdown-item detailcustomer" data-id="{{$v_customer->id}}" > Details</a>
                                                            @if(Auth::user()->id_role == 1 )
                                                            <a class="dropdown-item " href="{{ route('customers.fees', $v_customer->id)}}" > Fees</a>
                                                            <a class="dropdown-item " href="{{ route('customers.document', $v_customer->id)}}" > Documents</a>
                                                            <a class="dropdown-item " href="{{ route('customers.parameter', $v_customer->id)}}" > Parameters</a>
                                                            @endif
                                                            @if($v_customer->ref)
                                                            <a class="dropdown-item" href=""><span class="badge bg-warning">Recommended:{{$v_customer->ref}}</span></a>
                                                            @endif
                                                            @if($v_customer->Sellerdelete($v_customer->id) == 1)
                                                            <a class="dropdown-item deletecustomer" data-id="{{$v_customer->id}}" > Deleted</a>
                                                            @endif
                                                        </div>
                                                    </div> 
                                                </td>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <from class="form-horizontal form-material">
                                <div class="form-group">
                                    <div class="col-md-12 my-2">
                                        <select class="select2 form-control" id="detail_manager" style="width: 100%;height: 36px;">
                                            <option value=" ">Select Account Manager</option>
                                            @foreach($managers as $v_manager)
                                            <option value="{{ $v_manager->id}}">{{ $v_manager->name}}</option>
                                            @endforeach
                                    ...
                                    <div class="modal-body d-flex justify-content-center">
                                        <button type="button" class="btn btn-primary waves-effect mx-3" id="details">Save</button>
                                        <button type="button" class="btn btn-primary waves-effect" data-bs-dismiss="modal">Cancel</button>
                                    </div>
                                    @endif
                                </div>
        ...
        <script type="text/javascript">
            ...
            $(function () {
                ...
                $.get("{{ route('customers.index') }}" +'/' + customer +'/details', function (data) {
                        ...
                        $('#detail_manager').val(data.id_manager);
                ...
                $('#details').click(function(e){
                    ...
                    var manager = $('#detail_manager').val();
        ...
        <script type="text/javascript">
            $(function(e){
                ...
                $('#deletecustomer').click(function(e){
                    e.preventDefault();
                    if(confirm("Are you sure, you want to Deleted Seller?")){
                        var id = $(this).data('id');
                        $.ajax({
                            type : 'POST',
                            ...
                            success:function(response){
                                if(response.success == true){
                                    toastr.success('Good Job.', 'Seller Has been Deleted Success!', { "showMethod": "slideDown", "hideMethod": "slideUp", timeOut: 2000 });
                                    location.reload();
                                }
                            }
                        });
